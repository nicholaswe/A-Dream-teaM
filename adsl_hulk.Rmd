...
adsl_dm.2 = merge(adsl_dm.1,
                 
                 adsl.EXENDTC)


adsl.TRTEDT <- adsl_dm.2 %>% 
  mutate(TRTEDT =
  case_when (
...
# date of final dose (last EX record)

# looks like we need to sort by *EXSEQ*
...
# this gives us TREDT from EXENDTC from non-SF pts

# but, 6 pts need a value
# we get these from "date of discontinuation", if visit 3 was completed...

# check for these patients for discontinuation? sv?
...
vs.TRTEDT = vs %>% 
  ...
  filter(USUBJID %in% (ex_last.0 %>% filter(EXENDTC == ""))$USUBJID)
  
# visitnum for all is >3
# then, let's get the "date of discontinuation, from ds!
...
# so, we can take ds.DSSTDTC for the missing dates for these 6...

adsl.TREDT.k <- rbind(
  
  merge(adsl_dm,
      ex_last.0 %>% filter(EXENDTC != "") %>% select(USUBJID, EXENDTC)) %>% 
  
  rename(TRTEDT = EXENDTC),
  
  merge(adsl_dm,
      ds.TRTEDT %>% select(USUBJID, DSSTDTC)) %>% 
  
  rename(TRTEDT = DSSTDTC),
  
  adsl_dm %>% filter(DCDECOD == "SCREEN FAILURE") %>% 
...
adsl.TREDT.k %>% filter(USUBJID %in% (ex_last.0 %>% filter(EXENDTC == ""))$USUBJID) %>% 
...
adsl_dm.TRTSDT <- THISTHAT(adsl_dm, sv, "TRTSDT", "SVSTDTC", "VISITNUM", 3)


adsl_dm <- merge(adsl.TREDT.k,
...
adsl_trtdur <- adsl_dm %>% 
  ...
  mutate(TRTDUR = (TRTEDT %>% ymd)  - (TRTSDT %>% ymd) + 1)
